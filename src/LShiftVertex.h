...
#ifndef LSHIFTVERTEX_H_
#define LSHIFTVERTEX_H_
...
namespace vc_utils
{
    ...
    template < typename T = int, typename O = T >
    class LShiftVertex : public sc_core::sc_module, public Task_Base
    {
    public:
        //! \brief constructor with sc_time object
        explicit LShiftVertex( ProcessUnit_Base* _pUnit, name_t _name, unsigned int _vertexNumber,
              ...
              m_ProcessUnit( _pUnit )
        {
            ...
            sc_core::sc_spawn( sc_bind( &LShiftVertex::execute, this ),
                ( this->getName( ) + "_LShiftVertexProcess" ).c_str( ) );
            ...
            inputObs.addObserver( m_inputEvVec[ vc_utils::SIDE::RHS ].get( ),
                reinterpret_cast< dataPtr_t >( &m_inputTwoVal.second ), sizeof( unsigned int ) );
        }
        ...
        //! \brief destructor
        virtual ~LShiftVertex( ) = default;

    private:
        // forbidden constructors:
        LShiftVertex( ) = delete; //!< \brief because vertexID should be unique
        LShiftVertex( const LShiftVertex& _source ) =
            delete; //!< \brief because sc_module could not be copied
        LShiftVertex( LShiftVertex&& _source ) =
            delete; //!< \brief because move not implemented for sc_module
        LShiftVertex& operator=( const LShiftVertex& _source ) = delete; //!< \brief forbidden
        LShiftVertex& operator=( LShiftVertex&& _source ) = delete;      //!< \brief forbidden
    ...
    public:
        ...
        virtual const char* kind( ) const override { return "LShiftVertex"; }
    ...
    private:
        ...
        //! \var m_inputTwoVal
        //! \brief short description
        std::pair< unsigned int, unsigned int > m_inputTwoVal = {vc_utils::SIDE::RHS, 0};
