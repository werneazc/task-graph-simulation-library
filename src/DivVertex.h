//! \file DivVertex.h
//! \brief Task graph representation of an arithmetic devision
...
namespace vc_utils
{
    ...
    //! adds them. The result is casted into output data type.
    //!
    //! \tparam T data type of left hand side of division operator
    //! \tparam G data type of right hand side of division operator
    //! \tparam O output data type
    /************************************************************************/

    template < typename T = int, typename G = T, typename O = T >
    class DivVertex : public sc_core::sc_module, public Task_Base
    {
    public:
        //! \brief constructor with sc_time object
        explicit DivVertex( ProcessUnit_Base* _pUnit, name_t _name, unsigned int _vertexNumber,
              ...
              m_ProcessUnit( _pUnit )
        {
            ...
            sc_core::sc_spawn( sc_bind( &DivVertex::execute, this ),
                ( this->getName( ) + "_DivVertexProcess" ).c_str( ) );
        ...
        explicit DivVertex( ProcessUnit_Base* _pUnit, name_t _name, unsigned int _vertexNumber,
            ...
            : DivVertex( _pUnit, _name, _vertexNumber, _vertexColor, sc_time_t( _latency, _unit ) )
        {
        }

        //! \brief destructor
        virtual ~DivVertex( ) = default;

    private:
        // forbidden constructors:
        DivVertex( ) = delete; //!< \brief because vertexID should be unique
        DivVertex(
            const DivVertex& _source ) = delete; //!< \brief because sc_module could not be copied
        DivVertex(
            DivVertex&& _source ) = delete; //!< \brief because move not implemented for sc_module
        DivVertex& operator=( const DivVertex& _source ) = delete; //!< \brief forbidden
        DivVertex& operator=( DivVertex&& _source ) = delete;      //!< \brief forbidden

    public:
        /************************************************************************/
        // notifyObservers
        //! 
        //! \brief notify successors of division
        //!
    ...
    public:
        ...
        // execute
        //!
        //! \brief division of input value one by input value two
        //!
        //! \details 
        ...
        //! to be executed. 
        //! If the unit is free, the division is done and the process unit is released 
        //! by the this process.
    ...
    public:
        ...
        virtual const char* kind( ) const override { return "DivVertex"; }
