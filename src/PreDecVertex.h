...
#ifndef PREDECVERTEX_H_
#define PREDECVERTEX_H_
...
namespace vc_utils
{
    ...
    template < typename T = int > class PreDecVertex : public sc_core::sc_module, public Task_Base
    {
    public:
        //! \brief constructor with sc_time object
        explicit PreDecVertex( ProcessUnit_Base* _pUnit, name_t _name, unsigned int _vertexNumber,
              ...
              m_ProcessUnit( _pUnit )
        {
            ...
            sc_core::sc_spawn( sc_bind( &PreDecVertex::execute, this ),
                ( this->getName( ) + "_PreDecVertexProcess" ).c_str( ) );
        ...
        //! \brief destructor
        virtual ~PreDecVertex( ) = default;

    private:
        // forbidden constructors:
        PreDecVertex( ) = delete; //!< \brief because vertexID should be unique
        PreDecVertex( const PreDecVertex& _source ) =
            delete; //!< \brief because sc_module could not be copied
        PreDecVertex( PreDecVertex&& _source ) =
            delete; //!< \brief because move not implemented for sc_module
        PreDecVertex& operator=( const PreDecVertex& _source ) = delete; //!< \brief forbidden
        PreDecVertex& operator=( PreDecVertex&& _source ) = delete;      //!< \brief forbidden
    ...
    public:
        ...
        virtual const char* kind( ) const override { return "PreDecVertex"; }
