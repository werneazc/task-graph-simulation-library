...
namespace vc_utils
{
    ...
    template < typename T = int > class NotVertex : public sc_core::sc_module, public Task_Base
    {
    public:
        //! \brief constructor with sc_time object
        explicit NotVertex( ProcessUnit_Base* _pUnit, name_t _name, unsigned int _vertexNumber,
              ...
              m_ProcessUnit( _pUnit )
        {
            ...
            sc_core::sc_spawn( sc_bind( &NotVertex::execute, this ),
                ( this->getName( ) + "_NotVertexProcess" ).c_str( ) );
        ...
        explicit NotVertex( ProcessUnit_Base* _pUnit, name_t _name, unsigned int _vertexNumber,
            ...
            : NotVertex( _pUnit, _name, _vertexNumber, _vertexColor, sc_time_t( _latency, _unit ) )
        {
        }

        //! \brief destructor
        virtual ~NotVertex( ) = default;

    private:
        // forbidden constructors:
        NotVertex( ) = delete; //!< \brief because vertexID should be unique
        NotVertex(
            const NotVertex& _source ) = delete; //!< \brief because sc_module could not be copied
        NotVertex(
            NotVertex&& _source ) = delete; //!< \brief because move not implemented for sc_module
        NotVertex& operator=( const NotVertex& _source ) = delete; //!< \brief forbidden
        NotVertex& operator=( NotVertex&& _source ) = delete;      //!< \brief forbidden
            ...
            while ( true )
                {
                    ...
                    m_returnOneVal.second = static_cast< bool >( !m_inputOneVal.second );
    ...
    public:
        ...
        virtual const char* kind( ) const override { return "NotVertex"; }
