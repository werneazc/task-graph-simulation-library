//! \file PostIncVertex.h
//! \brief short description

#ifndef POSTINCVERTEX_H_
#define POSTINCVERTEX_H_
...
namespace vc_utils
{
    ...
    template < typename T = int > class PostIncVertex : public sc_core::sc_module, public Task_Base
    {
    public:
        //! \brief constructor with sc_time object
        explicit PostIncVertex( ProcessUnit_Base* _pUnit, name_t _name, unsigned int _vertexNumber,
              ...
              m_ProcessUnit( _pUnit )
        {
            ...
            sc_core::sc_spawn( sc_bind( &PostIncVertex::execute, this ),
                ( this->getName( ) + "_PostIncVertexProcess" ).c_str( ) );
        ...
        //! \brief destructor
        virtual ~PostIncVertex( ) = default;

    private:
        // forbidden constructors:
        PostIncVertex( ) = delete; //!< \brief because vertexID should be unique
        PostIncVertex( const PostIncVertex& _source ) =
            delete; //!< \brief because sc_module could not be copied
        PostIncVertex( PostIncVertex&& _source ) =
            delete; //!< \brief because move not implemented for sc_module
        PostIncVertex& operator=( const PostIncVertex& _source ) = delete; //!< \brief forbidden
        PostIncVertex& operator=( PostIncVertex&& _source ) = delete;      //!< \brief forbidden
    ...
    public:
        ...
        virtual const char* kind( ) const override { return "PostIncVertex"; }
