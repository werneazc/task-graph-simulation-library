...
namespace vc_utils
{
    ...
    class MulVertex : public sc_core::sc_module, public Task_Base
    {
    public:
        //! \brief constructor with sc_time object
        explicit MulVertex( ProcessUnit_Base* _pUnit, name_t _name, unsigned int _vertexNumber,
              ...
              m_ProcessUnit( _pUnit )
        {
            ...
            sc_core::sc_spawn( sc_bind( &MulVertex::execute, this ),
                ( this->getName( ) + "_MulVertexProcess" ).c_str( ) );
        ...
        explicit MulVertex( ProcessUnit_Base* _pUnit, name_t _name, unsigned int _vertexNumber,
            ...
            : MulVertex( _pUnit, _name, _vertexNumber, _vertexColor, sc_time_t( _latency, _unit ) )
        {
        }

        //! \brief destructor
        virtual ~MulVertex( ) = default;

    private:
        // forbidden constructors:
        MulVertex( ) = delete; //!< \brief because vertexID should be unique
        MulVertex(
            const MulVertex& _source ) = delete; //!< \brief because sc_module could not be copied
        MulVertex(
            MulVertex&& _source ) = delete; //!< \brief because move not implemented for sc_module
        MulVertex& operator=( const MulVertex& _source ) = delete; //!< \brief forbidden
        MulVertex& operator=( MulVertex&& _source ) = delete;      //!< \brief forbidden
    ...
    public:
        ...
        virtual const char* kind( ) const override { return "MulVertex"; }
