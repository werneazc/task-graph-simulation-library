...
#ifndef PREINCVERTEX_H_
#define PREINCVERTEX_H_
...
namespace vc_utils
{
    ...
    template < typename T = int > class PreIncVertex : public sc_core::sc_module, public Task_Base
    {
    public:
        //! \brief constructor with sc_time object
        explicit PreIncVertex( ProcessUnit_Base* _pUnit, name_t _name, unsigned int _vertexNumber,
              ...
              m_ProcessUnit( _pUnit )
        {
            ...
            sc_core::sc_spawn( sc_bind( &PreIncVertex::execute, this ),
                ( this->getName( ) + "_PreIncVertexProcess" ).c_str( ) );
        ...
        //! \brief destructor
        virtual ~PreIncVertex( ) = default;

    private:
        // forbidden constructors:
        PreIncVertex( ) = delete; //!< \brief because vertexID should be unique
        PreIncVertex( const PreIncVertex& _source ) =
            delete; //!< \brief because sc_module could not be copied
        PreIncVertex( PreIncVertex&& _source ) =
            delete; //!< \brief because move not implemented for sc_module
        PreIncVertex& operator=( const PreIncVertex& _source ) = delete; //!< \brief forbidden
        PreIncVertex& operator=( PreIncVertex&& _source ) = delete;      //!< \brief forbidden
    ...
    public:
        ...
        virtual const char* kind( ) const override { return "PreIncVertex"; }
