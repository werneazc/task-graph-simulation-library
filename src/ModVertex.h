...
namespace vc_utils
{
    ...
    class ModVertex : public sc_core::sc_module, public Task_Base
    {
    public:
        //! \brief constructor with sc_time object
        explicit ModVertex( ProcessUnit_Base* _pUnit, name_t _name, unsigned int _vertexNumber,
              ...
              m_ProcessUnit( _pUnit )
        {
            ...
            sc_core::sc_spawn( sc_bind( &ModVertex::execute, this ),
                ( this->getName( ) + "_ModVertexProcess" ).c_str( ) );
        ...
        explicit ModVertex( ProcessUnit_Base* _pUnit, name_t _name, unsigned int _vertexNumber,
            ...
            : ModVertex( _pUnit, _name, _vertexNumber, _vertexColor, sc_time_t( _latency, _unit ) )
        {
        }

        //! \brief destructor
        virtual ~ModVertex( ) = default;

    private:
        // forbidden constructors:
        ModVertex( ) = delete; //!< \brief because vertexID should be unique
        ModVertex(
            const ModVertex& _source ) = delete; //!< \brief because sc_module could not be copied
        ModVertex(
            ModVertex&& _source ) = delete; //!< \brief because move not implemented for sc_module
        ModVertex& operator=( const ModVertex& _source ) = delete; //!< \brief forbidden
        ModVertex& operator=( ModVertex&& _source ) = delete;      //!< \brief forbidden
    ...
    public:
        ...
        virtual const char* kind( ) const override { return "ModVertex"; }
