//! \file BitXorVertex.h
//! \brief Task graph representation of a binary XOR operation

#ifndef BITXORVERTEX_H_
#define BITXORVERTEX_H_
...
namespace vc_utils
{
    ...
    //!
    //! \tparam T data type of left hand side of binary XOR operator
    //! \tparam G data type of right hand side of binary XOR operator
    //! \tparam O output data type of result
    /************************************************************************/

    template < typename T = int, typename G = T, typename O = T >
    class BitXorVertex : public sc_core::sc_module, public Task_Base
    {
    public:
        //! \brief constructor with sc_time object
        explicit BitXorVertex( ProcessUnit_Base* _pUnit, name_t _name, unsigned int _vertexNumber,
              ...
              m_ProcessUnit( _pUnit )
        {
            ...
            sc_core::sc_spawn( sc_bind( &BitXorVertex::execute, this ),
                ( this->getName( ) + "_BitXorVertexProcess" ).c_str( ) );
        ...
        //! \brief destructor
        virtual ~BitXorVertex( ) = default;

    private:
        // forbidden constructors:
        BitXorVertex( ) = delete; //!< \brief because vertexID should be unique
        BitXorVertex( const BitXorVertex& _source ) =
            delete; //!< \brief because sc_module could not be copied
        BitXorVertex( BitXorVertex&& _source ) =
            delete; //!< \brief because move not implemented for sc_module
        BitXorVertex& operator=( const BitXorVertex& _source ) = delete; //!< \brief forbidden
        BitXorVertex& operator=( BitXorVertex&& _source ) = delete;      //!< \brief forbidden

    public:
        /************************************************************************/
        // notifyObservers
        //!
        //! \brief notify successors of binary operation
        //!
    ...
    public:
        ...
        //! to be executed. 
        //! If the unit is free, the binary XOR operation is done and the process unit is released 
        //! by the this process.
    ...
    public:
        ...
        virtual const char* kind( ) const override { return "BitXorVertex"; }
    ...
    private:
        ...
        //! \var m_inputOneVal
        //! \brief left hand side of binary XOR operation
        std::pair< unsigned int, T > m_inputOneVal = {vc_utils::SIDE::LHS, 0};

        //! \var m_inputTwoVal
        //! \brief right hand side of binary XOR operation
