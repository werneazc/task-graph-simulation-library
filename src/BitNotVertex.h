//! \file BitNotVertex.h
//! \brief Task graph representation of a binary NOT operation

#ifndef BITNOTVERTEX_H_
#define BITNOTVERTEX_H_
...
namespace vc_utils
{
    ...
    //!
    //! \details
    //! The vertex waits for the notification of his incoming value and
    //! binary NOT him.
    //! \tparam T data type of used value
    /************************************************************************/

    template < typename T = int > class BitNotVertex : public sc_core::sc_module, public Task_Base
    {
    public:
        //! \brief constructor with sc_time object
        explicit BitNotVertex( ProcessUnit_Base* _pUnit, name_t _name, unsigned int _vertexNumber,
              ...
              m_ProcessUnit( _pUnit )
        {
            ...
            sc_core::sc_spawn( sc_bind( &BitNotVertex::execute, this ),
                ( this->getName( ) + "_BitNotVertexProcess" ).c_str( ) );
        ...
        //! \brief destructor
        virtual ~BitNotVertex( ) = default;

    private:
        // forbidden constructors:
        BitNotVertex( ) = delete; //!< \brief because vertexID should be unique
        BitNotVertex( const BitNotVertex& _source ) =
            delete; //!< \brief because sc_module could not be copied
        BitNotVertex( BitNotVertex&& _source ) =
            delete; //!< \brief because move not implemented for sc_module
        BitNotVertex& operator=( const BitNotVertex& _source ) = delete; //!< \brief forbidden
        BitNotVertex& operator=( BitNotVertex&& _source ) = delete;      //!< \brief forbidden

    public:
        /************************************************************************/
        // notifyObservers
        //!
        //! \brief notify successor of binary NOT operation
        //!
    ...
    public:
        ...
        // execute
        //!
        //! \brief binary NOT operation on input value
        //!
        //! \details         
        ...
        //! to be executed. 
        //! If the unit is free, the binary NOT operation is done and the process unit is released 
        //! by the this process.
    ...
    public:
        ...
        virtual const char* kind( ) const override { return "BitNotVertex"; }
