//! \file AddVertex.h
//! \brief Task graph representation of an arithmetic addition
...
namespace vc_utils
{
    ...
    //!
    //! \tparam T data type of left hand side of add operator
    //! \tparam G data type of right hand side of add operator
    //! \tparam O output data type of sum
    /************************************************************************/

    template < typename T = int, typename G = T, typename O = T >
    class AddVertex : public sc_core::sc_module, public Task_Base
    {
    public:
        //! \brief constructor with sc_time object
        explicit AddVertex( ProcessUnit_Base* _pUnit, name_t _name, unsigned int _vertexNumber,
              ...
              m_ProcessUnit( _pUnit )
        {
            ...
            sc_core::sc_spawn( sc_bind( &AddVertex::execute, this ),
                ( this->getName( ) + "_AddVertexProcess" ).c_str( ) );
        ...
        explicit AddVertex( ProcessUnit_Base* _pUnit, name_t _name, unsigned int _vertexNumber,
            ...
            : AddVertex( _pUnit, _name, _vertexNumber, _vertexColor, sc_time_t( _latency, _unit ) )
        {
        }

        //! \brief destructor
        virtual ~AddVertex( ) = default;

    private:
        // forbidden constructors:
        AddVertex( ) = delete; //!< \brief because vertexID should be unique
        AddVertex(
            const AddVertex& _source ) = delete; //!< \brief because sc_module could not be copied
        AddVertex(
            AddVertex&& _source ) = delete; //!< \brief because move not implemented for sc_module
        AddVertex& operator=( const AddVertex& _source ) = delete; //!< \brief forbidden
        AddVertex& operator=( AddVertex&& _source ) = delete;      //!< \brief forbidden

    public:
        /************************************************************************/
        // notifyObservers
        //! 
        //! \brief notify successors of sum
        //!
    ...
    public:
        ...
        // execute
        //!
        //! \brief addition of input value one and two
        //!
        //! \details 
        ...
        //! to be executed. 
        //! If the unit is free, the addition is done and the process unit is released 
        //! by the this process.
    ...
    public:
        ...
        virtual const char* kind( ) const override { return "AddVertex"; }
    ...
    private:
        ...
        //! \var m_inputOneVal
        //! \brief left hand side of addition
        std::pair< unsigned int, T > m_inputOneVal = {vc_utils::SIDE::LHS, 0};

        //! \var m_inputTwoVal
        //! \brief right hand side of addition
